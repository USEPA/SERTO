...
class GaussianPlume(IDictable):
    ...
    def concentration(self, locations: np.array) -> np.array:
        ...
        if self.plume_type == self.PlumeType.PHYSICS_BASED:
            ...
            downwind_standard_dev = 1.0 # self.stability_coefficient *  (crosswind_norm ** self.stability_exponent)
            crosswind_standard_dev = self.stability_coefficient *  (downwind_norm ** self.stability_exponent)
